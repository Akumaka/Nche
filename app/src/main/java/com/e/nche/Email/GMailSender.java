            ...
            if (messages.length > inboxLength){
                ...
                for(int i = messages.length - 1, n = messages.length; i < n; i++) {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        ...
                        if (mail != null){
                            String ownerMail = recipients.substring(0, recipients.indexOf(","));
                            String driverMail = recipients.substring(recipients.indexOf(",") + 1, recipients.length() - 1);

                            if (mail.contains(ownerMail)){
                                Log.println(Log.ASSERT, "Status_Owner", ownerMail);

                                sent = false;
                            }

                            if (mail.contains(driverMail)){
                                Log.println(Log.ASSERT, "Status_Driver", driverMail);
